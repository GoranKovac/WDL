...
//NOTE: THIS WILL PROBABLE NEED TO BE REPLACED LATER WITH SWELL_OSWIDGET AT SOME POINT WHEN PROBLEM IS FOUND
...
static bool OnDragEventDelegate(GdkEvent *evt)
{
  ...
  switch (evt->type)
  {
    ...
    break;
    case GDK_DROP_START:
      //NOTE: WAYLAND this prevents drag and drop files from filemanagers
      // {
...
int SWELL_ShowCursor(BOOL bShow)
{
  ...
  if (s_cursor_vis_cnt==-1 && !bShow) 
  {
    ...
    //NOTE: WAYLAND needs to be relative in wayland or knobs are wonky (when cursor gets hidden)
